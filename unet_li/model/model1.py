...
class Unet(nn.Module):
    def __init__(self, in_channels, n_classes, phi=0):
        ...
        if 1 <= self.phi and self.phi <= 3:
            self.up1_att      = attention_block[self.phi - 1](256)
            self.up2_att      = attention_block[self.phi - 1](128)
            self.up3_att      = attention_block[self.phi - 1](64)
...
if __name__ == '__main__':
    ...
    device = 'cpu'
    net = Unet(in_channels=3, n_classes=1,phi=2)
