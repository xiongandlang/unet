...
parse.add_argument('--train_path', type=str, default=r'../train/image')
parse.add_argument('--val_path', type=str, default=r'../val/image')
...
def train(net: nn.Module, device: torch.device):
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.35, patience=2, verbose=True,
                                                     ...
                                                     eps=1e-08)  # 学习率调整器
    train_writer = SummaryWriter(log_dir=args.tensorboard_path+'/train', flush_secs=1)
    val_writer = SummaryWriter(log_dir=args.tensorboard_path+'/val', flush_secs=1)
    check_point = 0
    eval_iou_check = 0
    for epoch in range(args.epochs):
        ...
        pbar.close()
        train_iou = tp / (tp + fp + fn + 1e-08)
        train_precise = tp / (tp + fp + 1e-08)
        train_recall = tp / (tp + fn + 1e-08)
        train_f1 = 2 * train_precise * train_recall / (train_precise + train_recall + 1e-08)
        scheduler.step(train_iou)
        train_writer.add_scalar('loss', totalloss.item(), epoch)
        train_writer.add_scalar('iou', train_iou.item(), epoch)
        train_writer.add_scalar('f1', train_f1.item(), epoch)
        optimizer.zero_grad()
        if epoch+1 :
            net.eval()  # 测试模式
            tp, tn, fp, fn = 0, 0, 0, 0
            ...
            val_writer.add_scalar('loss', totalloss.item(), epoch)
            val_writer.add_scalar('iou', eval_iou.item(), epoch)
            val_writer.add_scalar('f1', eval_f1.item(), epoch)
            if eval_iou_check < eval_iou:
                eval_iou_check = eval_iou
                torch.save(net.state_dict(), 'model1.pth')
            logging.info(f'eval({epoch+1}) iou:{eval_iou} f1:{eval_f1}')
            logging.info(f'- - - - - - - - - - - - - - - - - - - - - - - - - \n')
